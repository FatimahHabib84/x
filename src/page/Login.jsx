...
function Login() {
    ...
    useEffect(() => {
        axios.get(`https://665736bb9f970b3b36c86669.mockapi.io/X`)
        .then(function(res){
    ...
    const handleClick = () => {
        ...
        if(user.email=='' || user.pwd==''){
            ...
            //     setData(res.data)
            // })
            // const validEmail = data.find(item=>item.email==user.email)
  ...
  return (
    <div className='flex flex-row max-sm:flex-col items-center justify-center px-8 min-w-screen bg-base-100 h-screen'>
        <div className='w-1/2 flex items-center justify-center  max-sm:mt-8'><img src={XLogo} /></div>
        <div className="card-body w-1/2  max-sm:mt-8  max-sm:w-full">
